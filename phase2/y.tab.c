...
{
       0,   117,   117,   133,   152,   158,   167,   175,   183,   188,
     195,   202,   216,   222,   237,   242,   252,   257,   262,   290,
     306,   328,   348,   358,   368,   374,   381,   388,   402,   407,
     435,   440,   484,   489,   522,   527,   560,   565,   604,   614,
     623,   630,   650,   655,   660,   665
};
#endif
...
//endp
							...
							(yyval)->code=(yyvsp[-2])->code+mainProc+ regAssign+ (yyvsp[-1])->code + endp +outdec + end  ;
							cout << endl;
						}
...
#line 262 "parser.y" /* yacc.c:1646  */
    {
					...
					*/
					...
					string goL1=L1+":\n";	
					string code4= (yyvsp[-3])->code;
					string movA="mov ax, "+string((yyvsp[-3])->getSymbol())+"\n";
					string cmp="cmp ax, 0\n";
					string goL2="je "+L2+"\n";	
					string code7= (yyvsp[0])->code;
					string code5= (yyvsp[-2])->code;	
					string jmp="jmp "+L1+"\n";
...
#line 306 "parser.y" /* yacc.c:1646  */
    {
					...
					(yyval)->code+="mov ax, "+string((yyvsp[-4])->getSymbol())+"\n";
					(yyval)->code+="cmp ax, 1\n";
...
#line 328 "parser.y" /* yacc.c:1646  */
    {
					...
					(yyval)=new SymbolInfo("while","nonterminal");
                                        char *label1=newLabel(); string L1 = string(label1);
                                        char *label2=newLabel(); string L2 = string(label2);
                                        string goL1=L1+":\n";  
                                        string code3=(yyvsp[-2])->code;
                                        string movA="mov ax, "+string((yyvsp[-2])->getSymbol())+"\n";
					string cmp="cmp ax, 0\n";                                    
					string je="je "+L2+"\n";
                                        string code5=(yyvsp[0])->code;	
                                        string jmpL1="jmp "+L1+"\n";
					string whileCode=  goL1 + code3+ movA+ cmp+ je+code5+ jmpL1  + L2+":\n";
                                        (yyval)->code+=whileCode;
...
#line 348 "parser.y" /* yacc.c:1646  */
    {
					...
					string mov  = "mov ax," + string((yyvsp[-2])->getSymbol()) + "\n"; 
...
#line 407 "parser.y" /* yacc.c:1646  */
    {
				...
				if((yyval)->arrIndexHolder==""){ //actualy it is more appropriate to use arrayLength to make decision
					if(string((yyvsp[-2])->getSymbol()) != string((yyvsp[0])->getSymbol()) )
					{
					...
					if(string((yyvsp[-2])->getSymbol()) != string((yyvsp[0])->getSymbol()) && (yyvsp[-2])->arrIndexHolder != (yyvsp[0])->arrIndexHolder  ){
					(yyval)->code+="mov ax, "+string((yyvsp[0])->getSymbol())+"\n";
...
#line 440 "parser.y" /* yacc.c:1646  */
    {
					...
					if(strcmp((yyvsp[-1])->getSymbol(),"&&")==0){
						...
						otherwise 0
						*/
						string mov1="mov ax, "+string((yyvsp[-2])->getSymbol())+"\n";
					        string cmp1="cmp ax, 0\nje "+L1+"\n";
					        
                                                string mov2="mov ax, "+string((yyvsp[0])->getSymbol())+"\n";
						...
						string first="mov ax, "+string((yyvsp[-2])->getSymbol())+"\n" + "cmp ax, 0\n" + "jne "+L1+"\n";
					        
                                                string second ="mov ax, "+string((yyvsp[0])->getSymbol())+"\n"
					        +"cmp ax, 1\n"
                                                +"je "+ L1+"\n";
...
#line 489 "parser.y" /* yacc.c:1646  */
    {
				...
				else if(strcmp((yyvsp[-1])->getSymbol(),"<=")==0){(yyval)->code+="jle " + string(label1)+"\n";
				}
				else if(strcmp((yyvsp[-1])->getSymbol(),">")==0){(yyval)->code+="jg " + string(label1)+"\n";
				}
				else if(strcmp((yyvsp[-1])->getSymbol(),">=")==0){(yyval)->code+="jge " + string(label1)+"\n";
				}
				else if(strcmp((yyvsp[-1])->getSymbol(),"==")==0){(yyval)->code+="je " + string(label1)+"\n";
				}
				else{
				}
...
#line 527 "parser.y" /* yacc.c:1646  */
    {
				...
				string add;
				if(string((yyvsp[-2])->getSymbol()) != zero && string((yyvsp[0])->getSymbol()) != zero)
				{
					if(strcmp((yyvsp[-1])->getSymbol(),"+")==0 ){ //checking whether any operand is 0 
						movA = "mov ax," + string((yyvsp[-2])->getSymbol()) + "\n"; 
						add= "add ax, " + string((yyvsp[0])->getSymbol()) +"\n"; string mov = "mov "+ T + ", ax\n";
						(yyval)->code += movA + add + mov;
						(yyval)->setSymbol(temp);
					}
					else if (strcmp((yyvsp[-1])->getSymbol(),"-")==0 ){
						movA = "mov ax," + string((yyvsp[-2])->getSymbol()) + "\n";
						add= "sub ax, " + string((yyvsp[0])->getSymbol()) +"\n"; string mov = "mov "+ T + ", ax\n";
						(yyval)->code += movA + add + mov;
						(yyval)->setSymbol(temp);
...
#line 565 "parser.y" /* yacc.c:1646  */
    {
						...
						//string($1->getSymbol()) != one  && string($3->getSymbol()) != one
							...
							char *temp=newTemp();
							if(strcmp((yyvsp[-1])->getSymbol(),"*")==0){
								if(string((yyvsp[-2])->getSymbol()) != one  && string((yyvsp[0])->getSymbol()) != one){ //if any one operand is 1 no code generation
								(yyval)->code += "mov ax, "+ string((yyvsp[-2])->getSymbol())+"\n";
								...
								(yyval)->code += clr + div + mov;(yyval)->setSymbol(temp);
